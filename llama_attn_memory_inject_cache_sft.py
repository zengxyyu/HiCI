...
group_size_ratio = 1/4  # Fraction of tokens in each group for local memory
...
class GlobalMemoryModule(nn.Module):
    ...
    Args:
        ...
        num_memory_slots: Number of learnable memory slots (default: 16)
    """
    ...
    def __init__(
        ...
        if False:
            # 从预训练嵌入中随机采样 num_memory_slots 个作为初始值
            # 理论依据: 预训练嵌入已经包含丰富的语义信息，比随机噪声好得多
            indices = torch.randperm(init_from_embeddings.size(0))[:num_memory_slots]
            self.memory_slots = nn.Parameter(init_from_embeddings[indices].clone())
            # ✅ 只在 rank 0 打印
            rank = dist.get_rank() if dist.is_initialized() else 0
            if rank == 0:
                print(
                    f"    ✅ Initialized memory_slots from pretrained embeddings (sampled {num_memory_slots} tokens)"
                )
        else:
            ...
            self.memory_slots = nn.Parameter(
                torch.randn(num_memory_slots, hidden_size) * std
            )
            rank = dist.get_rank() if dist.is_initialized() else 0
            if rank == 0:
...
class GlobalMemoryModule_Multi(nn.Module):
    """
    Learnable global memory for capturing document-level context.
    ...
    def __init__(
        ...
        if False:
            ...
            rank = dist.get_rank() if dist.is_initialized() else 0
            if rank == 0 and not GlobalMemoryModule_Multi._init_msg_printed:
                print(
                    f"⚠️  GlobalMemoryModule_Multi Fallback: Initialized memory_slots with std={std}"
                )
                GlobalMemoryModule_Multi._init_msg_printed = True
        ...
        if use_bottleneck:
            ...
            if rank == 0:
                print(
                    f"✅ GlobalMemoryModule_Multi: bottleneck_dim: {bottleneck_dim}, num_heads: {num_heads}"
                )
        ...
        if init_from_llama_attn is not None and not use_bottleneck:
            ...
            if rank == 0 and layer_idx == 0:
                print(
                    f"✅ [GlobalMemoryModule_Multi 方案C] Initialized Q/K/V projections from LLaMA pretrained weights"
                )
    ...
    def forward(self, hidden_states, attention_mask=None):
        ...
        Q_mem = Q_mem.view(
            bsz, self.num_memory_slots, self.num_heads, self.effective_head_dim
        )
        ...
        global_context = attn_output.view(
            bsz, self.num_memory_slots, self.effective_dim
        )
...
# region ===========================================================================
class GlobalMemoryModule_Flash(nn.Module):
    """
    Learnable global memory for capturing document-level context.
    ...
    def __init__(
        self, hidden_size, num_memory_slots=16, num_heads=32, init_from_embeddings=None
    ):
        super().__init__()
        self.hidden_size = hidden_size
        self.num_memory_slots = num_memory_slots
    ...
    def forward(self, hidden_states, attention_mask=None):
        ...
        Q_mem = Q_mem.view(bsz, self.num_memory_slots, self.num_heads, self.head_dim)
        ...
        if attention_mask is not None:
            ...
            output_unpad = flash_attn_varlen_kvpacked_func(
                ...
                cu_seqlens_q,  # [bsz + 1]
                cu_seqlens_kv,  # [bsz + 1]
                self.num_memory_slots,  # max_seqlen_q (固定)
            ...
            global_context = rearrange(
                output_unpad, "(b s) h d -> b s (h d)", b=bsz, s=self.num_memory_slots
            )
        else:
...
# endregion ===========================================================================
# 记忆类版本3
class GlobalMemoryModule_Flash_plus(nn.Module):
    """
    Learnable global memory for capturing document-level context.
    ...
    def __init__(
        ...
        if rank == 0:
            print(
                f"    [GlobalMemoryModule_Flash_plus] Initialized memory_slots with std={std}"
            )
            print(
                f"    [GlobalMemoryModule_Flash_plus] 复用 LLaMA K/V 投影，只有自己的 Q 投影"
            )
    ...
    def forward(self, key_states, value_states, attention_mask=None):
        ...
        改进版：接收已经投影+RoPE的 K/V，只需要投影自己的 memory queries。
...
#         num_memory_slots: Number of global memory slots (default: 16)
#         num_heads: Number of attention heads (default: 32)
...
#         # ====================================================================
#         self.memory_slots = nn.Parameter(
#             torch.empty(num_memory_slots, hidden_size)
#         )
...
#         k = max(int(seq_len * self.write_ratio), self.num_memory_slots)
...
#         Q_mem = self.q_proj(memory).view(
#             bsz, self.num_memory_slots, self.num_heads, self.head_dim
#         ).transpose(1, 2)
...
#             bsz, self.num_memory_slots, self.bottleneck_dim
#         )
...
class HybridGlobalMemory(nn.Module):
    """
    混合全局记忆 - 简化版（无 EMA）

    相比 HybridGlobalMemory 的改进：
    ...
    def _print_init_info(self):
        """打印初始化信息"""
        rank = dist.get_rank() if dist.is_initialized() else 0
        if rank == 0 and not HybridGlobalMemory._init_msg_printed:
            total_params = sum(p.numel() for p in self.parameters())
            print(f"   ✅ HybridGlobalMemoryClean initialized (无EMA简化版)")
            ...
            HybridGlobalMemory._init_msg_printed = True
...
# ============================================================================
# 方法3.2  🆕 共享压缩层优化版 - HybridGlobalMemoryShared
# ============================================================================
class HybridGlobalMemoryShared(nn.Module):
    """
    混合全局记忆 - 共享压缩层优化版
    ...
    def _print_init_info(self):
        """打印初始化信息"""
        rank = dist.get_rank() if dist.is_initialized() else 0
        if rank == 0 and not HybridGlobalMemoryShared._init_msg_printed:
            total_params = sum(p.numel() for p in self.parameters())
            ...
            print(f"   ✅ HybridGlobalMemoryShared initialized (共享压缩层优化版)")
            ...
            HybridGlobalMemoryShared._init_msg_printed = True
...
def forward_flashattn(
    ...
    - O(M×N + N) complexity where M=num_memory_slots (16), N=seq_len
    ...
    # Note: self.global_memory is registered via replace_llama_attn()
    # 传入 attention_mask 以正确处理 padding tokens
    global_context = self.global_memory(
        hidden_states, attention_mask
    )  # [bsz, num_slots, hidden_size]

    num_memory_slots = global_context.shape[1]
    ...
    global_q = (
        ...
        .view(bsz, num_memory_slots, self.num_heads, self.head_dim)
        .transpose(1, 2)
    )  # [bsz, nh, num_slots, hd]
    global_k = (
        self.k_proj(global_context)
        .view(bsz, num_memory_slots, self.num_key_value_heads, self.head_dim)
        .transpose(1, 2)
    )
    global_v = (
    ...
    chunk_len = num_memory_slots + group_size
...
def forward_flashattn_optimized(
    ...
    use_global_memory = hasattr(self, "global_memory")
    ...
    if use_global_memory:
        ...
        combined_q = (
            ...
            .view(bsz, num_memory_slots + q_len, self.num_heads, self.head_dim)
            .transpose(1, 2)
        ...
        combined_k = (
            ...
            .view(
                bsz, num_memory_slots + q_len, self.num_key_value_heads, self.head_dim
            )
            .transpose(1, 2)
        )
        ...
        global_q = combined_q[:, :, :num_memory_slots, :]  # [bsz, nh, num_slots, hd]
        query_states = combined_q[:, :, num_memory_slots:, :]  # [bsz, nh, q_len, hd]

        global_k = combined_k[:, :, :num_memory_slots, :]
        key_states = combined_k[:, :, num_memory_slots:, :]

        global_v = combined_v[:, :, :num_memory_slots, :]
        value_states = combined_v[:, :, num_memory_slots:, :]
    else:
    ...
    if use_global_memory:
        ...
        query_with_ctx[:, :, :, :num_memory_slots, :] = global_q.unsqueeze(
            2
        )  # broadcast across num_groups
        query_with_ctx[:, :, :, num_memory_slots:, :] = query_chunks

        key_with_ctx[:, :, :, :num_memory_slots, :] = global_k.unsqueeze(2)
        key_with_ctx[:, :, :, num_memory_slots:, :] = key_chunks

        value_with_ctx[:, :, :, :num_memory_slots, :] = global_v.unsqueeze(2)
        value_with_ctx[:, :, :, num_memory_slots:, :] = value_chunks
    ...
    if use_global_memory:
        # global_mask: [bsz, num_groups, num_memory_slots] - 全 1（memory 始终有效）
        global_mask = attention_mask.new_ones(bsz, num_groups, num_memory_slots)
...
def forward_flashattn_optimized_plus(
    ...
    """
    改进版：先做 Q/K/V 投影和 RoPE，再调用 GlobalMemoryModule。

    优势：
    1. GlobalMemoryModule 复用已投影的 K/V（节省一套投影）
    2. K/V 已有 RoPE 位置编码，记忆能感知位置
    ...
    if use_global_memory:
        ...
        # GlobalMemoryModule_Flash_plus 期望这个形状（与 Flash Attention 一致）
        key_for_memory = key_states.transpose(
            1, 2
        )  # [bsz, seq_len, num_heads, head_dim]
        value_for_memory = value_states.transpose(
            1, 2
        )  # [bsz, seq_len, num_heads, head_dim]

        # 调用 GlobalMemoryModule，它只投影自己的 Q，复用输入的 K/V
        global_context = self.global_memory(
            key_for_memory, value_for_memory, attention_mask
        )  # [bsz, num_slots, hidden_size]
...
#     # This captures document-level information before chunking
#     # Note: self.global_memory is registered via replace_llama_attn()
#     global_context = self.global_memory(hidden_states)  # [bsz, num_slots, hidden_size]
...
#     q_len_per_chunk = num_memory_slots + group_size  # e.g., 8 + 2048 = 2056

#     # K/V length: global + cache + chunk (WITH cache)
#     kv_len_per_chunk = num_memory_slots + self.recurrence_size + group_size  # e.g., 8 + 256 + 2048 = 2312
...
#             global_mask_q = torch.ones(bsz, num_memory_slots, dtype=attention_mask.dtype, device=attention_mask.device)
...
#             global_mask_kv = torch.ones(bsz, num_memory_slots, dtype=attention_mask.dtype, device=attention_mask.device)
...
#     # Structure for ALL chunks: [global(num_memory_slots), tokens(group_size)]
#     # Note: Q has NO cache positions, so skip_len is ONLY global
#     chunk_outputs = []
#     skip_len = num_memory_slots  # Only skip global (8), NO cache in Q!
...
def forward_flashattn_hierarchical_with_cache(
    ...
    """
    Hierarchical memory with cache support (完整整合版本).

    整合了以下功能：
    1. ✅ Recurrence cache (Transformer-XL style)
    2. ✅ Hierarchical memory (GlobalMemoryModule + HierarchicalMemoryAggregator)
    ...
    # 使用 GlobalMemoryModule 对每个 chunk 单独提取局部全局表示
    # ⚠️ CRITICAL: Check if global_memory exists before using it!
    if (use_higher_global or use_local_memory) and hasattr(self, "global_memory"):
        # 批处理所有 chunks（并行）
        ...
        # all_local_mems = self.global_memory(
    ...
    if (
        use_higher_global
        and hasattr(self, "hierarchical_aggregator")
        and local_memories_stacked is not None
    ):
        higher_global = self.hierarchical_aggregator(local_memories_stacked)
...
def forward_flashattn_hierarchical(
    ...
    整合了以下功能：
    1. Hierarchical memory (GlobalMemoryModule + HierarchicalMemoryAggregator)
    ...
    # ========== Step 1: 分 chunk ==========
    # ✅ SFT版本：动态计算能整除 q_len 的 group_size（无 padding，最高效）
    if q_len % 4096 == 0:
    ...
    # ========== Step 2: 提取局部记忆（对每个 chunk 提取压缩表示）==========
    # 使用 GlobalMemoryModule_Flash 对每个 chunk 单独提取局部全局表示
    if (use_higher_global or use_local_memory) and hasattr(self, "global_memory"):
        # 批处理所有 chunks（并行）
        all_chunks = chunks.view(bsz * num_groups, group_size, hidden_size)
...
#     # Global memory slots are position-independent, so they don't get RoPE
#     if position_ids is not None:
#         max_pos = position_ids.max().item() + 1
#         rope_seq_len = max(kv_seq_len, max_pos)
#     else:
...
#         total_seq_len = num_memory_slots + q_len
#     else:
#         total_seq_len = q_len
...
#     if global_q is not None:
#         # Skip the first num_memory_slots positions (global memory outputs)
#         # Keep only the sequence token outputs
#         output = output[:, num_memory_slots:, :, :]  # [bsz, q_len, nh, hd]
...
def forward_flashattn_full_hierarchical(
    ...
    # 记忆提取参数
    use_higher_global: bool = True,  # 是否使用高层全局记忆
    use_local_memory: bool = True,  # 是否使用局部记忆
    group_size_ratio: Optional[float] = 1,  # 用于分组提取记忆（不影响attention计算）
) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
    """
    评估方式3：Full Attention + Hierarchical Memory（无分组attention，无cache）

    特点：
    1. 记忆提取：可以分组提取局部记忆，再聚合为全局记忆
    2. Attention计算：Full Attention（不分组），将记忆拼接在序列前面
    3. 无 recurrence cache

    拼接结构：
    Q/K/V: [higher_global, local_memory, full_sequence]
    ...
    if not hasattr(self, "_eval_config_printed_way3"):
        ...
        if rank == 0 and layer_idx == 0:
            print("\n" + "=" * 80)
            print("📋 Evaluation Mode: Full Attention + Hierarchical Memory")
            print("=" * 80)
            print(f"  ✅ use_higher_global : {use_higher_global}")
            print(f"  ✅ use_local_memory  : {use_local_memory}")
            print(
                f"  ✅ group_size_ratio  : {group_size_ratio} (for memory extraction)"
            )
            print(f"  ❌ recurrence_cache  : disabled")
            print()
            print("📌 Attention Structure:")
            print("   Q/K/V: [global_memory, local_memory, full_sequence]")
    ...
    if (use_higher_global or use_local_memory) and hasattr(self, "global_memory"):
        ...
        all_local_mems = self.global_memory(all_chunks, attention_mask_chunks)
        num_local_slots = all_local_mems.shape[1]
        local_memories_stacked = all_local_mems.view(
            bsz, num_groups, num_local_slots, hidden_size
        )
    ...
    # 局部记忆（展平后的所有局部记忆）
    if use_local_memory and local_memories_flat is not None:
    ...
    # 计算总长度
    memory_len = num_global_slots + total_local_slots
    total_len = memory_len + q_len
    ...
    memory_mask = torch.ones(
        bsz, memory_len, dtype=attention_mask.dtype, device=attention_mask.device
    )
    full_mask = torch.cat([memory_mask, attention_mask], dim=1)
    ...
    seq_output = output[:, memory_len:, :, :]  # [bsz, q_len, nh, hd]
...
#        - 对每个 chunk 应用 GlobalMemoryModule
...
#     if hasattr(self, "global_memory") and hasattr(self, "hierarchical_aggregator"):
#         # 分 chunk 配置（必须与训练时一致）
#         chunk_size = 2048
...
#             # 应用 GlobalMemoryModule（与训练时相同）
#             local_mem = self.global_memory(
#                 chunk
#             )  # [batch, num_memory_slots, hidden_size]
#             local_memories.append(local_mem)

#         # Step 1.2: 聚合成全局记忆
#         all_local_memories = torch.cat(
#             local_memories, dim=1
#         )  # [batch, num_chunks*num_memory_slots, hidden_size]
#         higher_global = self.hierarchical_aggregator(
#             all_local_memories
#         )  # [batch, global_slots, hidden_size]
#         num_memory_slots = higher_global.shape[1]
...
#             .view(bsz, num_memory_slots, self.num_heads, self.head_dim)
#             .transpose(1, 2)
#         )  # [batch, num_heads, num_memory_slots, head_dim]

#         global_k = (
#             self.k_proj(higher_global)
#             .view(bsz, num_memory_slots, self.num_key_value_heads, self.head_dim)
#             .transpose(1, 2)
#         )  # [batch, num_kv_heads, num_memory_slots, head_dim]
...
def replace_llama_attn(
    ...
    use_optimized_plus_norope=False,  # 实验版：先调用 Memory 再应用 RoPE（SFT暂不支持）
    use_hierarchical_forward=False,  # 使用分层forward（局部+全局记忆）
):
    """
    ...
    To register GlobalMemoryModule parameters, call register_global_memory_to_model()
    ...
    Args:
        ...
        use_full: Whether to use full attention without chunking (default: False)
            - True: forward_flashattn_full (full attention, no memory)
            - False: forward_flashattn (chunked attention with memory, same as training)
        ...
        eval_mode: Evaluation mode selection (default: None)
            ...
            - "chunked": Chunked attention with memory (same as training)
            - "full": Full attention without memory
            - "full_hierarchical": Full attention + hierarchical memory (evaluation way3)
        ...
        use_optimized_plus: Whether to use the improved version (default: False)
            ...
            - 需要配合 register_global_memory_to_model(use_flash_plus=True) 使用
        ...
        use_hierarchical_forward: Whether to use hierarchical forward (default: False)
            - True: Use forward_flashattn_hierarchical (局部+全局记忆)
        ...
        if inference:
            ...
            if eval_mode == "full_hierarchical":
                ...
                if use_hierarchical_forward:
                    ...
                    if rank == 0:
                        print(
                            "   🧪 Using forward_flashattn_hierarchical (局部+全局记忆)"
                        )
                    ...
                    # 改进版：复用 LLaMA K/V 投影，先投影再调用 GlobalMemoryModule
                    # 需要配合 GlobalMemoryModule_Flash_plus 使用
                    transformers.models.llama.modeling_llama.LlamaAttention.forward = (
                        forward_flashattn_optimized_plus
                    )
                    if rank == 0:
...
# 控制参数的地方
def register_global_memory_to_model(
    model,
    num_memory_slots=16,
    global_slots=2,
    ...
    use_flash_plus=True,  # 新增：是否使用 GlobalMemoryModule_Flash_plus sh中控制
    use_flash: Optional[bool] = True,  # 是否使用 GlobalMemoryModule_Flash 在此处控制
    ...
    """
    Register GlobalMemoryModule (and optionally HierarchicalMemoryAggregator) to each LlamaAttention layer.
    ...
    Args:
        ...
        num_memory_slots: Number of local memory slots (for GlobalMemoryModule, default: 16)
        global_slots: Number of higher-level global slots (for HierarchicalMemoryAggregator, default: 16)
        num_heads: Number of attention heads (default: 32)
        bottleneck_dim: Bottleneck dimension for efficiency (default: 2048)
        use_hierarchical: If True, also register HierarchicalMemoryAggregator (default: False)
    ...
    Example usage in fine-tune.py:
        ...
        register_global_memory_to_model(model, num_memory_slots=16)

        # For hierarchical memory:
        register_global_memory_to_model(
            model,
            num_memory_slots=16,  # local slots
    ...
    if use_hierarchical and not use_local_summary:
        if rank == 0:
            ...
            print(
                "Reason: HierarchicalAggregator needs local memories from GlobalMemoryModule"
            )
            print()
        ...
        raise ValueError(
            ...
            "HierarchicalAggregator needs local memories from GlobalMemoryModule to aggregate."
        )
    ...
    for layer_idx, layer in enumerate(llama_model.layers):
        ...
        # Module 1: GlobalMemoryModule (always register)
        # ✅ 使用预训练嵌入初始化（方案 2）
        ...
        if use_local_summary:
            if use_flash_plus:
                ...
                attn.global_memory = GlobalMemoryModule_Flash_plus(
                ...
                attn.global_memory = GlobalMemoryModule_Flash(
                    hidden_size=hidden_size,
                    num_memory_slots=num_memory_slots,
                    num_heads=num_heads,
                    init_from_embeddings=embed_weight,
                ...
                # 配合 forward_flashattn 使用 GlobalMemoryModule_Multi
                # attn.global_memory = GlobalMemoryModule(
                #     hidden_size=hidden_size,
                #     num_memory_slots=num_memory_slots,
                #     num_heads=num_heads,
                #     init_from_embeddings=embed_weight,
                # ).to(model_dtype)
                attn.global_memory = GlobalMemoryModule_Multi(
                    hidden_size=hidden_size,
                    num_memory_slots=num_memory_slots,
                    num_heads=num_heads,
                    init_from_embeddings=embed_weight,
        ...
        if use_hierarchical:
            ...
            if use_shared_compressor:
                ...
                attn.hierarchical_aggregator = HybridGlobalMemoryShared(
                    hidden_size=hidden_size,
                ...
                # 原版：HybridGlobalMemory（方案B - ICML推荐：统计量 + Lightweight Attention）
                # 基于Information Bottleneck + Predictive Coding + RG理论
                # 参数量：13.7M/layer
                # 稳定性：✅✅✅ 极高（attention 在 5×512 低维空间）
                attn.hierarchical_aggregator = HybridGlobalMemory(
                    hidden_size=hidden_size,
                    global_slots=global_slots,
    ...
    global_memory_params = 0
    aggregator_params = 0

    if use_local_summary or use_hierarchical:
        # 遍历整个模型的参数（自动去重）
        for name, param in model.named_parameters():
            if "global_memory" in name:
                # 属于 GlobalMemory 的参数（包括 q_proj/k_proj/v_proj）
                global_memory_params += param.numel()
            elif "hierarchical_aggregator" in name:
    ...
    if rank == 0:
        ...
        if use_local_summary and use_hierarchical:
            total_memory_params = global_memory_params + aggregator_params
            print(f"\nRegistered Modules:")
            print(f"  ✓ Local Memory ({global_memory_params:,} params)")
            ...
            print(f"  ✓ Local Memory ({global_memory_params:,} params)")
            print(
                f"\nTotal Memory Params: {global_memory_params:,} ({global_memory_params / total_params * 100:.2f}%)"
            )
...
#     # region
#     """
#     Register GlobalMemoryModule to each LlamaAttention layer.

#     This MUST be called after model loading and before optimizer initialization!

#     Args:
#         model: LlamaForCausalLM or PeftModelForCausalLM
#         num_memory_slots: Number of learnable memory slots per layer (default: 16)
...
#         # 3. Register global memory (BEFORE optimizer!)
#         register_global_memory_to_model(model, num_memory_slots=16, recurrence_size=256)
...
#     # Register GlobalMemoryModule to each attention layer
#     for layer_idx, layer in enumerate(llama_model.layers):
#         attn = layer.self_attn

#         # Create and register GlobalMemoryModule as a sub-module
#         attn.global_memory = GlobalMemoryModule(
#             hidden_size=hidden_size,
#             num_memory_slots=num_memory_slots
#         )
...
#         print(f"Registered GlobalMemoryModule to layer {layer_idx} "
#               f"({num_memory_slots} slots × {hidden_size}D, recurrence_size={recurrence_size})")
...
#     print(f"   Per-layer config: {num_memory_slots} slots × {hidden_size}D = "
#           f"{num_memory_slots * hidden_size * 4:,} params × {len(llama_model.layers)} layers")
