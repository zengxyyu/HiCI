...
def forward_flashattn_full(
    ...
    Works for both Llama2 (num_kv_heads=32, groups=1) and Llama3 (num_kv_heads=8,
    groups=4). Uses flash_attn_func / flash_attn_varlen_kvpacked_func which handle
    different Q and KV head counts natively, avoiding repeat_kv memory overhead.
    Also fixes a prior bug where attention_mask=None would crash unpad_input.

    attention_mask: [bsz, q_len] padding mask (True = valid token), or None
    """
    if output_attentions:
        warnings.warn(
            "Output attentions is not supported for patched `LlamaAttention`, returning `None` instead."
    ...
    # No repeat_kv: flash_attn handles GQA natively (Q and KV head counts may differ)
    query_states = query_states.transpose(1, 2)   # [bsz, q_len, num_heads,    head_dim]
    key_states   = key_states.transpose(1, 2)     # [bsz, q_len, num_kv_heads, head_dim]
    value_states = value_states.transpose(1, 2)   # [bsz, q_len, num_kv_heads, head_dim]

    key_padding_mask = attention_mask

    if key_padding_mask is None or torch.all(key_padding_mask):
        # No padding: flash_attn_func handles GQA natively
        output = flash_attn_func(
            query_states, key_states, value_states,
            dropout_p=0.0, softmax_scale=None, causal=True,
        )
    else:
        # Has padding: unpad Q and KV separately (different head counts for GQA)
        q_unpad, indices_q, cu_seqlens_q, max_seqlen_q = unpad_input(
            rearrange(query_states, "b s h d -> b s (h d)"), key_padding_mask
        )
        q_unpad = rearrange(q_unpad, "nnz (h d) -> nnz h d", h=num_heads)

        kv     = torch.stack([key_states, value_states], dim=2)  # [bsz, q_len, 2, num_kv_heads, head_dim]
