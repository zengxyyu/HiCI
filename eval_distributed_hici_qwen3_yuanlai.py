...
class DistributedEvaluator:
    ...
    def evaluate(self, dataset, metric):
        ...
        with torch.no_grad():
            ...
            # Free GPU RAM before cross-device compute() to avoid OOM/SIGABRT
            del self.model
...
def run_eval(args: EvalArguments):
    ...
    if args.flash_attn:
        # Evaluation modes:
        # - None or "chunked": Chunked attention with HiCI (same as training) -- DEFAULT
        # - "full": Full attention without HiCI (standard transformer, baseline)
        ...
        if is_baseline:
            ...
            # Default: chunked attention with HiCI (same as training)
            replace_qwen3_attn(
                use_flash_attn=True,
                use_full=False,
                use_hierarchical_forward=True,
    ...
    # Determine whether HiCI modules are needed.
    # eval_mode="full" without peft_model = pure Qwen3 baseline, no HiCI needed.
    # All other cases need HiCI modules.
    need_hici = not (args.eval_mode == "full" and not args.peft_model)

    if need_hici:
        # Register HiCI modules (Qwen3)
        print(f"\n{'=' * 70}")
        print(f"Registering HiCI modules for Qwen3 Evaluation")
        print(f"{'=' * 70}")
        register_hici_to_qwen3_model(
            model,
            ...
            if checkpoint_files:
                ...
                for ckpt_file in checkpoint_files:
                    if os.path.isfile(ckpt_file):
                        ...
                        del state_dict
                print(f"Loaded {total_hici_keys} HiCI parameters")
            else:
        ...
        print(f"Skipping HiCI module registration and checkpoint loading")
        print(f"{'=' * 70}\n")
