...
_TRANSFORMERS_NEW = Version(transformers.__version__) >= Version("4.45")
...
class ChunkLlamaRotaryEmbedding(nn.Module):
    def __init__(self, dim=None, max_position_embeddings=4096, base=10000, scaling_factor=1.0, device=None, config=None):
        super().__init__()

        # Support both old-style (dim, max_position_embeddings, ...) and new-style (config=config)
    ...
    def forward(self, x, seq_len=None):
        ...
        # In transformers >= 4.51, LlamaModel calls rotary_emb(hidden_states, position_ids)
        # where position_ids is a tensor. This result is unused by ChunkLlama's attention forward
        # (which calls self.rotary_emb with seq_len=int directly), so return dummy values.
...
def forward(
    ...
    # transformers >= 4.45: DecoderLayer unpacks 2 values; older versions expect 3
    if _TRANSFORMERS_NEW:
...
def _patch_attn_init(original_init):
    """Wrap LlamaAttention.__init__ to add missing attributes and per-layer rotary_emb
    for compatibility with transformers >= 4.51."""
    def patched_init(self, *args, **kwargs):
        ...
        # In transformers >= 4.51, rotary_emb lives on LlamaModel, not on each attention layer.
        # ChunkLlama's forward accesses self.rotary_emb, so create one per layer.
        if not hasattr(self, 'rotary_emb'):
            self.rotary_emb = ChunkLlamaRotaryEmbedding(config=self.config)
    return patched_init
...
def replace_with_chunkllama(pretraining_length=4096, local_window_size=None):
    ...
    llama_mod.LlamaAttention.__init__ = _patch_attn_init(llama_mod.LlamaAttention.__init__)
    llama_mod.LlamaAttention.forward = forward
    if hasattr(llama_mod, 'LlamaFlashAttention2'):
        llama_mod.LlamaFlashAttention2.forward = forward
    llama_mod.LlamaRotaryEmbedding = ChunkLlamaRotaryEmbedding
    if hasattr(llama_mod, 'LlamaLinearScalingRotaryEmbedding'):
        llama_mod.LlamaLinearScalingRotaryEmbedding = ChunkLlamaRotaryEmbedding
