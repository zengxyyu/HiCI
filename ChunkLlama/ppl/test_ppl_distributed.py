# -*- coding:utf-8 -*-
"""
Multi-GPU distributed PPL evaluation for ChunkLlama (DCA).
Uses the same torchmetrics.Perplexity as eval_distributed_hici_qwen3.py for consistent comparison.

Usage:
    torchrun --nproc_per_node=4 ChunkLlama/ppl/test_ppl_distributed.py \
...
class Pg19Dataset(Dataset):
    def __init__(self, data_path, seq_length, sliding_window=256, data_dtype="uint16"):
...
def main():
    ...
    if args.model_path:
        ...
        model_path = os.path.join(os.path.dirname(__file__), f"../../models/Llama-2-{args.scale}-hf")
    ...
    # Enable distributed evaluation (same trick as eval_distributed_hici_qwen3.py)
    ...
    # Same metric as eval_distributed_hici_qwen3.py
